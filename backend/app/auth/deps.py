"""Request-scoped dependencies: database session, current user, RBAC guards.

`get_db` carries the preventive half of audit enforcement: a mutating
request whose transaction is about to commit with zero recorded audit
events is rolled back and refused. The middleware in `app.audit.middleware`
is the detection backstop for anything that bypasses this dependency.
"""
...
def get_db(request: Request) -> Iterator[Session]:
    """Yield the request transaction; refuse un-audited state changes."""
...
def get_current_user(
    ...
    """Resolve the active session token to a user with roles and privileges."""
...
def get_recorder(
    ...
    """An audit recorder bound to the request transaction and acting user."""
